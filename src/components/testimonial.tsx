...
const testimonials = [
  {
    ...
    review: "ZunoSync cut my content creation time by 80%. I used to spend hours on captions — now it takes seconds.",
  },
  {
    name: "Mike Rivera",
    role: "Social Media Manager",
    image: "/images/testimonials/avatar2.jpg",
    review: "Managing 5 client accounts was a nightmare. ZunoSync helps me stay consistent across all of them without burning out.",
  },
  {
    name: "Priya Patel",
  ...
  },
  {
    ...
    review: "No more staring at a blank screen. I type a few words and get multiple post angles instantly. Game changer.",
  },
  {
    name: "Mitchel Ryan",
    role: "Consultant",
...
export default function Testimonial() {
  return (
    <section className="relative bg-[#0b0510] text-white py-24 px-6 overflow-hidden">
      {/* BG glow blobs */}
      ...
      <div className="relative z-10 max-w-7xl mx-auto">
        ...
        <div className="text-center mb-20">
          ...
          <motion.span
            ...
            className="px-5 py-2 border border-white/30 rounded-full text-[13px] font-bold uppercase tracking-widest text-white inline-block mb-8"
          >
            {badgeText}
          </motion.span>

          {/* H2 — letter by letter + blur */}
          <h2 className="text-4xl md:text-5xl lg:text-6xl font-bold mt-2 mb-6 tracking-tight font-exo">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 items-start">
          ...
          <div className="flex flex-col gap-6">
            {[testimonials[1], testimonials[4]].map((item, i) => (
              <TestimonialCard key={i} item={item} colDelay={0.15} rowDelay={i * 0.15} tall />
            ))}
          </div>
          {/* Column 3 */}
          <div className="flex flex-col gap-6">
...
function TestimonialCard({ item, colDelay, rowDelay, tall = false }: { item: any; colDelay: number; rowDelay: number; tall?: boolean }) {
  ...
  return (
    <motion.div
      ...
      className={`relative bg-gradient-to-b from-white/[0.05] to-white/[0.01] border border-white/[0.08] rounded-[32px] p-8 hover:bg-white/[0.06] transition-all duration-500 group${tall ? " min-h-[400px]" : ""}`}
    >
      {/* Stars */}
      <div className="flex items-center justify-between mb-8">
        <div className="flex gap-1">
          ...
          <span className="text-white text-2xl relative">
            ★
